...
var _ = Describe("ZeroTrustWorkloadIdentityManager Controller", func() {
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			By("creating the custom resource for the Kind ZeroTrustWorkloadIdentityManager")
			err := k8sClient.Get(ctx, typeNamespacedName, zerotrustworkloadidentitymanager)
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance ZeroTrustWorkloadIdentityManager")
			Expect(k8sClient.Delete(ctx, resource)).To(Succeed())
		})
		It("should successfully reconcile the resource", func() {
			By("Reconciling the created resource")
			controllerReconciler := &ZeroTrustWorkloadIdentityManagerReconciler{
				Client: k8sClient,
